...
def mcmc(model_params, model_hyperparams, real_infected, temperature=1, iterations=20000, chains=6, \
    ...
    for i in range(iterations):
        ...
        new_chi += np.array([0.5*((new_params[c][7] - 0.16667)/0.016667)**2 for c in range(chains)])
        new_chi += np.array([0.5*((new_params[c][8] - 0.003428)/0.0003428)**2 for c in range(chains)])
        new_chi += np.array([0.5*((new_params[c][9] - 0.025)/0.005)**2 for c in range(chains)])
